...
use crate::{Result, ProgressBarTheme};
...
impl FocusModeApp {
    ...
    fn get_color(&self, progress: f64) -> Color {
        match self.theme {
            ProgressBarTheme::Simple => Color::White,
...
pub fn run_focus_mode(duration: Duration, name: &str, theme: ProgressBarTheme, bell: bool, notify: bool) -> Result<()> {
    enable_raw_mode()?;
...
fn run_app<B: tui::backend::Backend>(
    ...
    loop {
        terminal.draw(|f| {
            ...
            let inner_area = timer_area.inner(&Margin { vertical: 1, horizontal: 1 });
            ...
            let time_text = if app.paused {
                format!("PAUSED - {} remaining", format_simple_duration(app.remaining()))
            } else {
            ...
            let time_paragraph = Paragraph::new(time_text)
                ...
                .style(
                    Style::default()
                        .fg(if app.paused { Color::Yellow } else { Color::White })
                );
